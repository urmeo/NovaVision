...
% ---- Title block, NeurIPS preprint form ----
\begin{center}
{\LARGE\bfseries The Probe Is the Bottleneck: Zero-Shot Round-Trip Recovery\\[2pt]
Does Not Yet Certify Emotion Controllability\\[2pt]
in Text-to-Image Generation\par}
\vspace{14pt}
{\bfseries Urme Bose}\\[2pt]
{\small\texttt{github.com/urme-b/NovaVision}}
\end{center}
\vspace{6pt}
...
Round-trip recovery is the default certificate of emotion controllability in text-to-image generation: condition on an emotion, classify the generated image, count the matches. Zero-shot CLIP makes the loop free, so the loop is everywhere. We test whether it certifies anything. Our protocol holds image content independent of the intended emotion and bounds every number with a chance floor, a template ceiling, a shuffled-label permutation control, family-wise correction, and probe error deconvolved into the estimate. A CPU pilot (SD-Turbo, CLIP ViT-B/32, $n=14$ per tier) returns a guarded null: no conditioning tier beats the shuffled-label control (Holm-adjusted $p=0.27$), and the emotion tier's apparent lift, 0.214 over a 0.143 chance floor, corrects to 0.165 once the probe's measured error is removed. The failure is the instrument. In domain the probe collapses onto \emph{neutral}, using 2 of 7 labels and assigning neutral to 90\% of items; on real scenes it reads emotion at only 40.3\% (ViT-L/14: 45.5\%, McNemar $p=0.038$). We release AffectBench, the harness, and a preregistered powered run ($n=420$ per tier, $\geq$95\% power) that executes only after a probe demonstrates in-domain competence.
\end{list}
\vspace{4pt}

\section{Motivation}
Emotion-conditioned image generation now has a name and a leaderboard. EmoGen defined the task and the metric the field defaults to: generate on an emotion, classify the image, count the matches (Yang et al., 2024). EmotiCrafter extended conditioning to continuous valence and arousal (Dang et al., 2025); adapters and guidance followed (Yuan et al., 2025; Xia et al., 2025); training-free prompt conditioning arrived with EPIG (Othmen et al., 2026). The applications are not hypothetical: affective media, accessibility, tools that respond to how a sentence feels rather than what it names.

Every such system faces one question. Did the image convey the emotion, or did we only intend it to? Humans answer slowly and expensively. The round trip answers instantly, and with zero-shot CLIP it answers for one forward pass per image. That cheapness is exactly why the assumption deserves a hard test: a free measurement spreads before anyone checks what it measures. Generative evaluation has been here before. FID contradicts human raters on modern text-to-image models (Jayasumana et al., 2024). Most fidelity metrics fail basic sanity checks (Naeem et al., 2020). No standard metric strongly tracks human judgment at scale (Stein et al., 2023). Emotion round-trip recovery has never received the same scrutiny. This paper supplies it, in runnable form.

\section{Gap}
Three omissions make published round-trip numbers unreadable.

\textbf{No controls.} Accuracy is reported bare: no chance floor, no template ceiling, no test against random label agreement. The failure mode is obvious once stated. If conditioning paints a fixed per-emotion scene and the probe is worded to match it, recovery is a tautology; the number measures prompt-probe vocabulary overlap, not the image.
...
\textbf{No preregistration.} Runs are sized and analyzed after the fact. A null becomes indistinguishable from an underpowered shrug; a positive, from a garden of forking paths.

The metric-critique tradition closed these gaps for FID and Inception Score. For emotion controllability they stand open, and they matter more here: perceived emotion is subtler than fidelity, and the default probe is weaker.

\section{Hypothesis}
The hypotheses are frozen in the repository's preregistration before the powered run. The committed pilot is a calibration of the instrument, not a test of them.

\begin{itemize}\itemsep1pt
\item \textbf{H1 (conditioning beats chance).} For each conditioning tier (naive, emotion, affect), recovery accuracy exceeds the shuffled-label permutation null (one-sided, 2000 permutations).
\item \textbf{H2 (conditioning beats the template).} The emotion and affect tiers add recovery over the scene template ceiling on the content track.
...
\item \textbf{RQ2.} Does any candidate probe clear an in-domain competence gate: accuracy materially above the majority baseline on real emotional scenes, more than two of seven labels used on generated ones? Candidates: CLIP ViT-B/32, ViT-L/14, and an EmoSet-trained classifier through the same interface.
\item \textbf{RQ3.} Does any apparent lift survive Rogan-Gladen correction for the probe's measured error?
\item \textbf{RQ4 (secondary).} On the text track, where valence and arousal are grounded in the sentence rather than a per-emotion prior, do continuous affect correlations show signal that categorical recovery misses?
\end{itemize}

\section{Methodology}

\subsection{Pipeline and conditioning tiers}
Text enters a DistilRoBERTa emotion classifier (Hartmann, 2022) over the six Ekman emotions plus neutral (Ekman, 1992). The emotion is grounded in Russell's circumplex (Russell, 1980): valence and arousal come from a per-emotion prior blended with lexical affect from an affect lexicon (empirical norms: Warriner et al., 2013; Mohammad, 2018; the shipped default is a small offline demo) as $v = c\,v_{\mathrm{lex}} + (1-c)\,v_{\mathrm{prior}}$, with lexical coverage $c$ capped at 0.8 so a fully in-lexicon input never discards the prior. Scoring skips function words and applies a two-token negation flip to valence. The blend is a stated heuristic, ablatable at $c=0$ and $c=1$, and flagged as an open validity question.
...
The content track uses twenty affect-neutral subjects. The text track uses AffectBench, built on demand from GoEmotions (Demszky et al., 2020) under the official Ekman grouping: balanced per-class sampling from the test split, exact within-sample dedup, subtraction of any item whose normalized text appears in the train split, counts and a content hash logged, composition and licensing documented in a datasheet (Gebru et al., 2021). GoEmotions is Reddit-English with modest Ekman-level agreement ($\kappa \approx 0.33$ to $0.44$); the benchmark inherits that skew, and the scope is text-prompt controllability, stated as such.

\subsection{Statistics}
Every tier reports recovery accuracy with a bootstrap 95\% CI, macro-F1, valence and arousal correlations with CIs, CLIP-T for content preservation, the majority-class baseline, a probe-collapse diagnostic (how many labels the probe ever uses), and the shuffled-label $p$-value. Holm correction controls the family-wise rate across tiers (Holm, 1979); Cohen's $h$ accompanies every contrast, so significance never travels without magnitude. Because the probe is imperfect, apparent recovery is deconvolved through the Rogan-Gladen estimator (Rogan and Gladen, 1978) using the probe's measured sensitivity and specificity: instrument error becomes a correction, not a caveat. Provenance is total. Each run logs git SHA, library versions, pinned model revisions, device, dtype, and the benchmark hash.

\subsection{The pilot as proof of concept}
The repository ships a committed CPU pilot (256\,px, 2 subjects, 1 seed; $n=14$ per tier, $n=7$ for scene; the naive tier postdates the committed pilot and has no committed numbers yet). Its purpose is calibration: run the protocol end to end and measure the instrument. It did both, and the second finding governs everything.

\begin{table}[t]
\centering\small
\begin{tabular}{l c c c c}
\hline
\textbf{Tier} & \textbf{Accuracy [95\% CI]} & \textbf{Macro-F1} & \textbf{Shuffled $p$} & \textbf{Holm $p$} \\
\hline
raw & 0.143 [0.000, 0.357] & 0.038 & 0.86 & n/a \\
emotion & 0.214 [0.000, 0.429] & 0.112 & 0.23 & 0.27 \\
affect & 0.214 [0.000, 0.429] & 0.133 & 0.14 & 0.27 \\
scene & 0.286 [0.000, 0.575] & 0.184 & 0.14 & n/a \\
\hline
\end{tabular}
\caption{Pilot recovery by tier. Chance and the majority baseline coincide at 0.143 (1/7).}
\label{tab:results}
\end{table}
...
\caption{Recovery accuracy by tier in the committed pilot. Every interval includes the 1/7 chance line.}
\label{fig:accuracy}
\end{figure}
...
\caption{Raw-tier confusion from the pilot: predictions collapse onto the neutral row.}
\label{fig:confusion}
\end{figure}

Probe validation locates the failure. Out of domain (faces, $n=200$), ViT-B/32 reads Ekman emotion at 29.0\%, with bimodal recall: neutral 0.81 and anger 0.61 against surprise 0.04, fear 0.06, sadness 0.08. In domain on real scenes (EmoSet, $n=400$) it reaches 40.3\% while emitting all seven labels; ViT-L/14 reaches 45.5\% (faces: 37.5\%), significant per set by exact paired McNemar test ($p=0.038$ on scenes, discordant 36 vs.\ 57; $p=0.040$ on faces). Two consequences follow. The probes are not blind on real photographs, so the neutral collapse is a property of the generated images (256\,px, 2-step SD-Turbo) interacting with the probe; the generator's affective weakness is a co-suspect. And a structural suspect sits underneath both: image-emotion resources standardize on Mikels' eight categories with no neutral (Mikels et al., 2005; Yang et al., 2023), while the targets here are Ekman plus neutral, and neutral is exactly where the probe collapses.

The pilot did what a Stage~1 calibration should: it proved the harness runs, and it caught its own instrument failing before that instrument could manufacture a conclusion. The repository makes the catch checkable, with pinned revisions, run manifests, a 200-test suite, a CI target that re-derives every reported number from the committed records, and a schema-validated submission format for scoring other generators and probes under the frozen protocol.
...
1 & Gate candidate probes in domain: ViT-L/14, then an EmoSet-trained classifier via the probe interface & More than 2 of 7 labels on generated scenes; accuracy materially above majority & In progress; L/14 registered candidate (45.5\%, McNemar $p=0.038$) \\
...
\item \textbf{AffectBench and an open harness}: a frozen, datasheeted, dedup-checked GoEmotions-derived benchmark; pinned models, total run provenance, a 200-test suite with CI-locked numbers, and a submission schema for scoring any generator or probe under identical rules.
...
\refentry{Hartmann, J. (2022). Emotion English DistilRoBERTa-base. Hugging Face model.}
