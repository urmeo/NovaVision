...
> The Results tables and figures are produced by the reproducible pipeline
> (`make reproduce && make paper`); `paper/tables.md` is generated, never
> hand-written. Numbers below are inserted from that artifact.
...
confidence intervals and a paired significance test. The contribution is the validated protocol
and artifact, not a single controllability score; we also ship a probe-validation and
human-study harness so the proxy can be checked against people.
...
AffectBench is built on demand from GoEmotions [@demszky2020] under the official Ekman grouping:
single-label examples are mapped to seven Ekman categories, deduplicated, sampled per class from
the **test** split (so items do not overlap model training splits), and interleaved so any prefix
stays balanced. The build records realized per-class counts and a content hash; a datasheet
(`data/DATASHEET.md`) documents composition, splits, and licensing. A hand-authored sample ships
only as a test fixture and can never produce a reported number — `load_benchmark` has no default
path. Because GoEmotions is *text* emotion, results conditioned on it are scoped to text-prompt
controllability; image-grounded affect (EmoSet/FI) is used to validate the probe.
...
- **Valence/arousal correlation** — Pearson $r$ and Spearman $\rho$ between intended and probed
  affect, reported with the caveat that on neutral content the intended axis varies between
  emotions; within-emotion variance is the honest target.
...
Tables 1 and 2 are generated by `scripts/report.py` from the canonical run and included from
`paper/tables.md`; per-tier confusion matrices and valence/arousal scatters are in
...
The probe is a proxy for perceived emotion and is validated two ways. First, the same probe is
run on a labelled image-emotion set (FI/EmoSet) and its accuracy and confusion are reported as
the instrument's known error. Second, a human study (`novavision.eval.human_study`) regenerates a
stratified sample of images, collects labels from three or more raters, and reports human-vs-probe
agreement (Cohen's $\kappa$) and human-vs-intended accuracy. Every downstream number is read
against that bound. The harness ships; populated numbers accompany the canonical run.
