...
configuration is `make reproduce` (see below). (This committed pilot predates the `naive` tier, so
its tables show `raw`/`emotion`/`affect`/`scene` and three contrasts; a fresh `make pilot` adds the
`naive` baseline and its two contrasts.) `make paper` regenerates the tables from whichever
...
| Contrast | Δ accuracy | 95% CI | Cohen's h | p |
|---|---|---|---|---|
